...
import { useAddProductMutation, useUpdateProductMutation } from "./productsApiSlice"
...
const ManageProducts = ({ visible, setVisible, iceCream }) => {
    // console.log("add product");
    // const window=Window()
    const navigate=useNavigate()
    const [addProduct, { isError, isLoading, isSuccess, data, error, refetch }] = useAddProductMutation()
    ...
    const [formData, setformData] = useState(iceCream?{
        ...iceCream
    }:{
        name: "",
        price: 0,
            ...
            content={({ hide }) => (
                <div className="flex flex-column px-8 py-5 gap-4" style={{ borderRadius: '12px', backgroundImage: 'radial-gradient(circle at left top, var(--primary-400), var(--primary-700))' }}>
                    ...
                    <div className="inline-flex flex-column gap-2">
                        <FloatLabel>
                            <InputText required id="name" className="inputD" name="name" value={iceCream&&formData.name} onChange={(e) => handleChange(e)}></InputText>
                            <label htmlFor="name">שם</label>
                        </FloatLabel>
                        <FloatLabel>
                            <InputText required id="price" className="inputD" name="price" value={iceCream&&formData.price} onChange={(e) => handleChange(e)}></InputText>
                            <label htmlFor="price">מחיר</label>
                        </FloatLabel>
                        <FloatLabel>
                            <InputText required id="quantity" className="inputD" name="quantity" value={iceCream&&formData.quantity} onChange={(e) => handleChange(e)}></InputText>
                            <label htmlFor="quantity">כמות</label>
                        </FloatLabel>
                        <FloatLabel>
                            <InputText id="image" className="inputD" name="image" value={iceCream&&formData.image} onChange={(e) => handleChange(e)}></InputText>
                            <label htmlFor="image">תמונה</label>
                        </FloatLabel>
                        <div className="flex align-items-center gap-2">
                            <Button className="inputD" onClick={handleSubmit} label={iceCream?'עדכן':"הוסף"} />
                            <Button className="inputD" label="ביטול" onClick={(e) => hide(e)} />
                        </div>
